...
async function populateOverview() {
	try {
		...
		// Animar os stages com delay
		await updateStagesWithAnimationDelayed(currentStageIndex);
...
// ========================================
// STAGE ANIMATION WITH DELAYS
// ========================================

async function updateStagesWithAnimationDelayed(currentStageIndex) {
	const stagesList = document.querySelector("#overviewContent .stages-list");

	if (!stagesList) {
		console.warn("Stages list not found");
		return;
	}

	const stageItems = stagesList.querySelectorAll(".stage-item");
	const DELAY = 50; // 500ms entre cada stage
	...
	for (let i = 0; i < stageItems.length; i++) {
		...
		stageItem.style.transform = "translateY(8px)";
		stageItem.style.transition = "all 0.2s ease";

		// Esperar fade out completar
		await new Promise((resolve) => setTimeout(resolve, 200));

		// Atualizar as classes
		stageItem.classList.remove("active", "upcoming", "completed");
		stageItem.classList.add(newStatus);

		// Fade in
		stageItem.style.opacity = "1";
		stageItem.style.transform = "translateY(0)";
		stageItem.style.transition = "all 0.3s ease";

		// Esperar fade in + delay para próximo stage
		await new Promise((resolve) => setTimeout(resolve, 300));

		// Esperar o delay antes do próximo stage (exceto no último)
		if (i < stageItems.length - 1) {
